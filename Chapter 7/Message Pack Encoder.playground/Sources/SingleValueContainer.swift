...
extension _MessagePackEncoder {
    // SingleValueContainer 类负责编码 nil、true、false、 整型、浮点数、字符串、日期（例如时间戳）和 data（例如字节数组）类型。
    // 每一个 SingleValueContainer 对象都通过编码器的 codingPath 和 userInfo 来初始化。 codingPath 属性可以跟踪编码器的内部状态，在调试的时候会用得上。 userInfo 属性提供让用户可以通过额外的配置选项来改变编码器的行为。
